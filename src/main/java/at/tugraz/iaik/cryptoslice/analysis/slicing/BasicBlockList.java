...
public class BasicBlockList {
   ...
   */
  public BasicBlockList(BasicBlock firstBB, byte[] pathState, boolean isBackwardSearch) {
    path.add(firstBB);
   ...
   */
  public BasicBlockList(LinkedList<BasicBlock> previousPath, byte[] pathState, boolean isBackwardSearch) {
    this.previousPath = previousPath;
    this.isBackwardSearch = isBackwardSearch;
    // the last one is the starting BB
    BasicBlock last = this.previousPath.removeLast();
    path.add(last);
    this.pathState.add(pathState);
   ...
   * @param pathState Can be used to denote the tracked register for the current BB, it can be overridden if it changes while handling new BBs
   */
  public BasicBlockList(BasicBlock firstBB, byte[] pathState) {
   ...
   * @param pathState Can be used to denote the tracked register for the current BB, it can be overridden if it changes while handling new BBs
   */
  public BasicBlockList(LinkedList<BasicBlock> previousPath, byte[] pathState) {
  ...
  public BasicBlock getNextBb() {
    ...
    BasicBlock current;
    BasicBlock retBB = null; // the bb to be returned
    ...
    while (!path.isEmpty() && retBB == null) { // search until retBB is found or no more BBs are available
      ...
      for (BasicBlock bb : blocks) { // search next successor
   ...
   * @return the path
   */
  public LinkedList<BasicBlock> getPathForLastBB() {
    LinkedList<BasicBlock> ret = new LinkedList<>();
  ...
  public String toString() {
    ...
    sb.append("\nPREV PATH: ");
    for (BasicBlock bb : previousPath) {
      sb.append(bb.getLabel());
      sb.append("\t");
    }
    sb.append("\nisBackwards: ");
